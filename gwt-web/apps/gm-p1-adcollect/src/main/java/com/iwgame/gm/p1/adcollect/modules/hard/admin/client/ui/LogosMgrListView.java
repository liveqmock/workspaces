...
import com.iwgame.gm.p1.adcollect.modules.hard.admin.client.ui.dialog.OperationLogosDialog;
 ...
 * @创建时间：2012-10-10 下午02:58:02
 */
public class LogosMgrListView extends SchemaGridView {

	private LogosMgrPresenter presenter;

	/**
	 * @return 获取 presenter
	 */
	public LogosMgrPresenter getPresenter() {
		return presenter;
	}
	 ...
	 *            设置 presenter
	 */
	public void setPresenter(final LogosMgrPresenter presenter) {
	...
	public LogosMgrListView() {
		...
		addBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(final ClickEvent event) {
				// 调用添加的Dialog
				new OperationLogosDialog(true, LogosMgrListView.this).center();
			}
		});
		...
		updateBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(final ClickEvent event) {
				if ((getPanel().getGrid().getSelected() == null)
					...
					MessageBox.alert("请选择一个合同Logo！");
				} else if (getPanel().getGrid().getSelected().size() > 1) {
					MessageBox.alert("只能选择一个合同Logo修改信息！");
				} else {
					// 调用修改的 Dialog
					new OperationLogosDialog(false, LogosMgrListView.this).center();
				}
			}
		});
		...
		deleteBtn.addClickHandler(new ClickHandler() {
			...
			public void onClick(final ClickEvent event) {
				if ((getPanel().getGrid().getSelected() == null)
					...
					MessageBox.alert("只能选择一个合同Logo删除信息！");
				} else {

					MessageBox.confirm("确认删除"
							+ getGrid().getSelected().get(0).get("name").toString(),
							new MessageBoxHandler() {
								...
								public void onClose(final MessageBoxEvent event) {
									...
									if (button.getItemId().equals(XDialogBox.YES)) {
										...
										presenter.deleteLogos(d.intValue(),
