 ...
 * 
 */
public class ChannelDialog extends XDialogBox {
	...
	private ChannelPortField fldChannelPort;
	...
	private void buildBasicForm() {
		...
		fldChannelId.setDescription("  (格式: flow-[xxxx].{guid}.{gameid}."
				+ ApplicationContext.getCurrentProduct().getName().toLowerCase() + ")");
		...
		fldChannelPort = new ChannelPortField("分配端口");
...
//		layout.add(fldChannelPort);
		...
		fldCNodeRoom.setDelegate(new Delegate<FlumeNode>() {
			...
			@Override
			public void loadItem(final AsyncCallback<List<FlumeNode>> callback) {
				ConfigService.Util.get().retrieveFlumeNodes(ApplicationContext.getCurrentProduct().getName(), "sink",
						callback);
		...
		// JSONValue aconfig = fldANodeConfig.getValue();
		// aconfig.isObject().put("sink", new
		// JSONString(ANODE_SINK.replace("${mqsink}", mqSink)));
		// fldANodeConfig.setValue(aconfig);
		// }
		// });
	...
	private void buildFetcherForm() {
		...
		fldANodeConfig.addValidator(new Validator() {
			...
			public String validate(final Field<?, ?> field) {
				...
				boolean hasEmptyNode = "".equals(json.isObject().get("anodeId").isString().stringValue())
						|| "".equals(json.isObject().get("flowId").isString().stringValue())
						|| "".equals(json.isObject().get("source").isString().stringValue())
						|| "".equals(json.isObject().get("sink").isString().stringValue());
				if (hasEmptyNode) {
	...
	private void buildSinkForm() {
		...
		fldCNodeConfig.addValidator(new Validator() {
			...
			public String validate(final Field<?, ?> field) {
				...
				boolean hasEmptyNode = "".equals(json.isObject().get("cnodeId").isString().stringValue())
						|| "".equals(json.isObject().get("flowId").isString().stringValue())
						|| "".equals(json.isObject().get("source").isString().stringValue())
						|| "".equals(json.isObject().get("sink").isString().stringValue());
				if (hasEmptyNode) {
	...
	public ChannelDialog(final SchemaGridView view, final FlumeChannel channel) {
...
//		tab.add(sinkForm, "收集节点信息");
		...
		if (channel != null) {
			...
			fldANodeConfig.setValue("".equals(channel.getaNodeConfig()) ? ANODE_ID_FLOW_ID_SOURCE_SINK : channel
					.getaNodeConfig());
			fldCNodeConfig.setValue("".equals(channel.getcNodeConfig()) ? CNODE_ID_FLOW_ID_SOURCE_SINK : channel
					.getcNodeConfig());
