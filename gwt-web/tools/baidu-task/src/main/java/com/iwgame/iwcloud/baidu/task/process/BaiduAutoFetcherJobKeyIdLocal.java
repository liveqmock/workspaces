 ...
 * @日期:	2012-10-29上午11:40:49
 * @版本:   v1.0
 */
public class BaiduAutoFetcherJobKeyIdLocal extends BaiduAuto {

	private final Logger logger = Logger.getLogger(BaiduAutoFetcherJobKeyIdLocal.class);
	...
	public void work() {
		...
		try {
			logger.info("[Local][ KeyId 百度数据抓取任务] 开始。。。");
			logger.info("[Local] KeyId 当前任务抓取时间范围：[ " + DateTimeUtil.formatDate3DateTimeStr(fetcherConfig.getFetcherBeginTime()) + " ]---[ "
			...
			if (null != adsconfigs && adsconfigs.size() > 0) {
				for (int i = 0; i < adsconfigs.size(); i++) {
					...
					try {
						...
						adsHistoryBean.setUsername("[Local]"+adsConfig.getUsername());
						...
						while(trycount-- > 0){
							try {
								...
								logger.error("[Local] KeyId 获取ReportId失败,webservice连接超时,等待10秒我们将重试一次,请稍候...  \t 异常:",e);
								Thread.sleep(10000);
							}
						}
						...
						if (reprotIdResponse != null) {
							...
							while(trynum-- >0){
								try {
									...
									//尝试一次停止30秒
									logger.error("[Local] 获取fileUrlResponse失败,webservice连接超时,我们将重试一次,请稍候...  \t 异常:",e);
								}
							}
							...
							if (null != fileUrlResponse) {
								...
								if (content != null && !"".equals(content)) {
									...
									for (int j = 0; j < list.size(); j++) {
										...
										try {
											...
											// 调存储过程
											reslut = adsService.sp_store_baidu_keyid_ads_Local(map, adsConfig.getProductid());
											if (reslut == 0) {
												info = "[Local] [SYNC SUCCESS]" + adsConfig.getProductid() + "->" + adslog.getAccount();
												successcount++;
											}
										} catch (Exception e) {
											logger.error("[Local] 调用存储过程异常！", e);
											info = "[Local][SYNC FAILED]" + adsConfig.getProductid() + "->" + adslog.getAccount() + ",ResultCode:" + reslut;
											logger.error(info);
											throw e;
										}
									}
								...
								status = -1;
								info += "[Local] KeyId 获取广告失败,当前帐号: [ " + adsConfig.getUsername() + " ]!  百度帐号是否正确?,百度的webservice 是否连接超时?";
								logger.error(info);
							}
						} else {
							status = -1;
							info = "[Local] KeyId reprotIdResponse 对象为空，请检测百度帐号配置！";
							logger.error(info);
						}
					} catch (Exception e) {
						status = -1;
						info += "[Local] KeyId 获取广告失败,当前帐号: [ " + adsConfig.getUsername() + " ]!  百度帐号是否正确?,百度的webservice 是否连接超时?";
						logger.error("[Local] KeyId Ignore Fetcher for Account : [ " + adsConfig.getUsername() + " ] !");
						logger.error(info, e);
					}
					...
					logger.info("[Local] -----------End fetcher KeyId [Account:" + adsconfigs.get(i).getUsername() + "]-------------\n\n\n");
				}
			} else {
				logger.info("[Local] Get Baidu-ADS Account for Zero!");
			}
		} catch (Exception e) {
			logger.error("[Local]获取百度帐号配置失败!", e);
		}
		...
		for (int j = 0; j < adsHistoryBeans.size(); j++) {
			...
			logger.info("[Local] KeyId帐号: [" + bean.getUsername() + "]  抓取：[" + bean.getFetchecount() + "] 保存：[" + bean.getSavecount() + "] 是否成功?：[" + success + "]");
		}
		logger.info("--------------------------------------");
		logger.info("[Local][KeyId百度数据抓取任务] 结束。。。\n\n\n\n\n");
