 ...
 * @日期:	2012-10-29上午11:40:59
 * @版本:   v1.0
 */
public class BaiduAutoFetcherJobLocal extends BaiduAuto{

	private final Logger logger = Logger.getLogger(BaiduAutoFetcherJobLocal.class);
	...
	public void work() {
		...
		try {
			logger.info("[Local百度数据抓取任务] 开始。。。");
			logger.info("[Local]当前任务抓取时间范围：[ " + DateTimeUtil.formatDate3DateTimeStr(fetcherConfig.getFetcherBeginTime()) + " ]---[ "
					...
					try {
						...
						while(trycount-- > 0){
							try {
								...
								reprotIdResponse = service.getProfessionalReportId(fetcherConfig);
								break;
							} catch (Exception e) {
								logger.error("[Local]获取ReportId失败,webservice连接超时,我们将重试一次,请稍候...  \t 异常:",e);
								Thread.sleep(10000);
							}
						}
						...
						if (reprotIdResponse != null) {
							...
							if (null != fileUrlResponse) {
								...
								if (content != null && !"".equals(content)) {
									...
									for (int j = 0; j < list.size(); j++) {
										...
										try {
											...
											// 存入本地
											reslut = adsService.sp_store_baidu_ads_Local(map, adsConfig.getProductid());
								...
								status = -1;
								info += "[Local]获取广告失败,当前帐号: [ " + adsConfig.getUsername() + " ]!  百度帐号是否正确?,百度的webservice 是否连接超时?";
								logger.error(info);
							}
						} else {
							status = -1;
							info = "[Local] reprotIdResponse 对象为空，请检测百度帐号配置！";
							logger.error(info);
						}
					} catch (Exception e) {
						status = -1;
						info += "[Local] 获取广告失败,当前帐号: [ " + adsConfig.getUsername() + " ]!  百度帐号是否正确?,百度的webservice 是否连接超时?";
						logger.error("[Local] Ignore Fetcher for Account : [ " + adsConfig.getUsername() + " ] !");
						logger.error(info, e);
					}
					adsHistoryBean.setStatus(status);
					adsHistoryBean.setFetchecount(fetchecount);
					adsHistoryBean.setSavecount(successcount);
					adsHistoryBean.setInfo(info);
					adsHistoryBeans.add(adsHistoryBean);
					// 存入历史记录
					adsService.insertAdshistory(adsHistoryBean, null);
					logger.info("[Local] -----------End fetcher [Account:" + adsconfigs.get(i).getUsername() + "]-------------\n\n\n");
				}
			} else {
		...
		logger.info("[Local]详细信息：");
		logger.info("-------------------------------------");
		for (int j = 0; j < adsHistoryBeans.size(); j++) {
			AdsHistoryBean bean = adsHistoryBeans.get(j);
			String success = bean.getStatus() == 0 ? "成功" : "失败";
			logger.info("[Local]帐号: [" + bean.getUsername() + "]  抓取：[" + bean.getFetchecount() + "] 保存：[" + bean.getSavecount() + "] 是否成功?：[" + success + "]");
		}
		logger.info("--------------------------------------");
		logger.info("[Local][百度数据抓取任务] 结束。。。\n\n\n\n\n");
