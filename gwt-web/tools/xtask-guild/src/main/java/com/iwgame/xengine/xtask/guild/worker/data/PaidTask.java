...
/**
 * @ClassName: PaidTask
 * @Description: 参赛玩家充值数据
 * @author 吴江晖
 * @date 2012-6-21 下午02:51:35
 * @Version 1.0
 * 
 */
public class PaidTask extends AbstractMQWorker {

	private static final Logger logger = Logger.getLogger(PaidTask.class);
	...
	public synchronized void handleMessage(final String text) {
		logger.info("######################    充值数据计算  开始  ######################");
		logger.info("从MQ收到充值数据");
		...
		try {
			...
			if (joiner != null) {
				...
				dataService.addPaid(productId, username, money);
			} else {
				logger.info("抛弃数据！");
			}

		} catch (Exception e) {
			logger.error("充值数据计算错误。" + e.getMessage(), e);
		} finally {
			logger.info("######################    充值数据计算  结束  ######################");
		}
