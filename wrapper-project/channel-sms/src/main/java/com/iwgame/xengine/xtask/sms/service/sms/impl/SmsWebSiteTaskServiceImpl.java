 ...
 * @日期: 2012-10-26上午11:50:46
 * @版本: v1.0
 */
@Service("smsWebSiteTaskService")
public class SmsWebSiteTaskServiceImpl extends SmsCoreSender implements SmsXtaskService {

	private final Logger logger = Logger.getLogger(SmsWebSiteTaskServiceImpl.class);
	...
	public String SMSSending(String phone, String message) throws SocketTimeoutException, ClientProtocolException, UnknownHostException, IOException {
		...
		try {
			// 生成单一下行请求消息 url
			String urlstr = CommonUtils.generateSingleSmsMtUrl("spid7947", "sppassword7947", phone, message);
			...
			if (resStr != null && !"".equals(resStr)) {
				...
				if (!"".equals(mtmsgid) && "000".equals(mterrcode)) {
					...
					logger.error("[蜀门/醉逍遥官网短信下发通道_短信发送失败],消息状态:[ " + mtstat + " ],服务器返回状态code:" + mterrcode + ",请求资源[phone:" + phone + ",message:" + message + "]");
					return resStr;
				}
			} else {
				// 这种情况不在重发短信，有可能导致堵塞 。但是记录日志
				logger.error("[蜀门/醉逍遥官网短信下发通道_短信发送失败],返回的解析字符串为空或NULL,请求资源[phone:" + phone + ",message:" + message + "]");
				return resStr;
			}
			...
			logger.error("[蜀门/醉逍遥官网短信下发通道_短信发送失败],其他异常,请求资源[phone:" + phone + ",message:" + message + "] 其他异常信息：", e);
			return resStr;
		}
	}
	...
	public String SMSGroupSending(String phones, String message) throws SocketTimeoutException, ClientProtocolException, UnknownHostException, IOException {
		...
		try {
			// 群发下行请求消息 url
			String urlstr = CommonUtils.generateSmsMultiMtUrl("spid7947", "sppassword7947", phones, message);
			...
			if (resStr != null && !"".equals(resStr)) {
				...
				if ((!"".equals(mtmsgid) || !"".equals(mtmsgids)) && "000".equals(mterrcode)) {
					logger.info("[蜀门/醉逍遥官网短信下发通道_短信发送成功],消息状态:[ " + mtstat + " ],请求资源[phone:" + phones + ",message:" + message + "]");
					return SUCCESS;
				} else {
					// 这种情况不在重发短信，有可能导致堵塞。但是记录日志
					logger.error("[蜀门/醉逍遥官网短信下发通道_短信发送失败],服务器返回状态code:" + mterrcode + ",请求资源[phone:" + phones + ",message:" + message + "]");
					return resStr;
				}
			} else {
				// 这种情况不在重发短信，有可能导致堵塞 。但是记录日志
				logger.error("[蜀门/醉逍遥官网短信下发通道_短信发送失败],返回的解析字符串为空或NULL,请求资源[phone:" + phones + ",message:" + message + "]");
				return resStr;
			}
		} catch (ClientProtocolException e) {
			throw e;
			...
			logger.error("[蜀门/醉逍遥官网短信下发通道_短信发送失败],其他异常,请求资源[phone:" + phones + ",message:" + message + "] 其他异常信息：", e);
			return resStr;
		}
