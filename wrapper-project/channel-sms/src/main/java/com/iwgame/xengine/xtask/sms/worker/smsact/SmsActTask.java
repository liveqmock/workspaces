 ...
 * @日期: 2012-10-26上午09:40:37
 * @版本: v1.0.2
 */
@Component
public class SmsActTask extends AbstractMQWorker {

	@Resource
	private SmsXtaskService smsActTaskService;
	 ...
	 */
	public void handleSmsAct(String source) throws IOException {

		logger.info("[活动系统短信下发通道]请求资源：" + source);
		...
		try {
			...
			if (phones != null && phones.length > 1) {
				if (falg) {
					...
					// 短信群发
					String result = smsActTaskService.SMSGroupSending(group.toString(), sms.getMessage());
					if ("success".equalsIgnoreCase(result)) {
						logger.info("短信发送成功-> Phones: " + group.toString());
						monitorlog.setLognote("活动短信群发成功!");
						monitorlog.setLogtype(0);
					} else {
						logger.error("活动短信群发失败,忽略! phones: " + group.toString() + "服务器返回：" + result);
						monitorlog.setLognote("活动短信群发失败，忽略! 服务器返回：" + result);
					}
				} else {
				...
				if (falg) {
					...
					if (isPhone) {
						...
						String result = smsActTaskService.SMSSending(sms.getPhone(), sms.getMessage());
						if ("success".equalsIgnoreCase(result)) {
							logger.info("短信发送成功-> Phones: " + sms.getPhone());
							monitorlog.setLognote("活动短信发成功!");
							monitorlog.setLogtype(0);
						} else {
						...
						monitorlog.setLognote("手机号不合法,忽略请求..!");
					}
				} else {
					logger.error("必要要字段为空,请求:" + source);
					monitorlog.setLognote("必要要字段为空,请求忽略...");
				}
			}
			...
			logger.error("活动系统短信下发通道_短信发送失败:" + sms.toString() + " 异常:", e);
			monitorlog.setLognote("[活动系统短信下发通道]短信发送失败,忽略,其他异常" + e.getMessage());
		} finally {
			try {
				xtaskTools.saveXtaskLogInfo(monitorlog);
			} catch (Exception ex) {
				logger.error("[活动系统短信下发通道]保存日志失败!");
			}
		}
	}
}
