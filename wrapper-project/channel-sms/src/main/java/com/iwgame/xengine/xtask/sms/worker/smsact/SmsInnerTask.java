 ...
 * @日期: 2012-10-26上午09:39:54
 * @版本: v1.0.2
 */
@Component
public class SmsInnerTask extends AbstractMQWorker {

	private final Logger logger = Logger.getLogger(SmsInnerTask.class);

	@Resource
	private SmsXtaskService smsInnerTaskService;

	@Resource
	private XtaskTools xtaskTools;

	public void handleSmsInner(String source) throws IOException {

		logger.info("[内部短信下发通道]请求资源：" + source);
		...
		try {
			...
			if (phones != null && phones.length > 1) {
				if (falg) {
					...
					// 短信群发
					String result = smsInnerTaskService.SMSGroupSending(group.toString(), sms.getMessage());
					if ("success".equalsIgnoreCase(result)) {
						logger.info("短信发送成功-> Phones: " + group.toString());
						monitorlog.setLognote("群发送短信成功!");
						monitorlog.setLogtype(0);
					} else {
						logger.error("活动短信群发失败,忽略! Phones: " + group.toString() + "服务器返回：" + result);
						monitorlog.setLognote("短信群发失败! 服务器返回：" + result);
					}
				} else {
				...
				if (falg) {
					...
					if (isPhone) {
						...
						String result = smsInnerTaskService.SMSSending(sms.getPhone(), sms.getMessage());
						if ("success".equalsIgnoreCase(result)) {
							logger.info("短信发送成功-> Phones: " + sms.getPhone());
							monitorlog.setLognote("发送短信成功!");
							monitorlog.setLogtype(0);
						} else {
							logger.error("活动短信发送失败,忽略! phones: " + sms.getPhone() + "服务器返回：" + result);
							monitorlog.setLognote("活动短信发失败! 服务器返回：" + result);
						}
					} else {
