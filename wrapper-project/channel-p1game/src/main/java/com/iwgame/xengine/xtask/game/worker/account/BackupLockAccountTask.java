 ...
 */
public class BackupLockAccountTask extends AbstractMQWorker {
	...
	public void handleMessage(String source) throws Exception {
		...
		logger.info("帐号封停,冻结[备份通道]收到请求资源: " + source);
		...
		try {
			...
			try {
				...
				monitorlog.setLognote("[FROM备份通道][帐号封停]成功！");
				xtaskTools.saveXtaskLogInfo(monitorlog);
			} catch (Exception e) {
				
			}
		} catch (Exception e) {
			logger.error("[帐号封停-备用资源发放异常,间隔时间10秒]：当前请求资源：[" + source + "]ExceptionInfo:" + e);
			try {
				Thread.sleep(10000);
