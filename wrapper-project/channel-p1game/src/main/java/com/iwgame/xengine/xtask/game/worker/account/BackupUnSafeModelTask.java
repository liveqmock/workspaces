 ...
 * @版本: v1.0
 */
public class BackupUnSafeModelTask extends AbstractMQWorker{
	...
	public void handleMessage(String source) throws Exception {
		...
		try {
			...
			try {
				...
				monitorlog.setLognote("[FROM备用通道][解除安全模式]成功！");
				xtaskTools.saveXtaskLogInfo(monitorlog);
			} catch (Exception e) {
			}
		} catch (Exception e) {
			logger.error("[解除安全模式-备用通道发放异常,等待10秒]：当前请求资源：[" + source + "] 异常:" + e);
			try {
				Thread.sleep(10000);
