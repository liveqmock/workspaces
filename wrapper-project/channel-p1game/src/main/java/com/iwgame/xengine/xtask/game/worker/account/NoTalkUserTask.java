 ...
 */
public class NoTalkUserTask extends AbstractMQWorker {
	...
	public void handleMessage(String source) {
		...
		try {
			...
			if (checking) {
				...
				monitorlog.setLognote("[用户禁言]成功！");
			}else{
				logger.error("禁言请求的必要参数为空或者NULL,忽略此条消息...");
				monitorlog.setLognote("禁言请求的必要参数为空或者NULL,忽略此条消息...");
			}
		} catch (BadSqlGrammarException e) {
			monitorlog.setLognote("[用户禁言]失败,发送到备份通道,连接异常"+e.getMessage());
			gameTaskServiceImpl.reSendMQBackupNoTalkUserActivate(source);
			...
			monitorlog.setLognote("[用户禁言]出错,忽略,其他异常"+e.getMessage());
			logger.error("其他异常信息,请求资源" + source, e);
		}finally {
			try {
				xtaskTools.saveXtaskLogInfo(monitorlog);
			} catch (Exception ex) {
				logger.error("[xtask-support][用户禁言]存储日志异常！",ex);
			}
		}
	}
}
