 ...
 * @日期: 2012-10-26上午09:40:17
 * @版本: v1.0.2
 */
@Component
public class SmsWebSiteTask extends AbstractMQWorker {

	private final Logger logger = Logger.getLogger(SmsWebSiteTask.class);

	@Resource
	private SmsXtaskService smsWebSiteTaskService;

	@Resource
	private XtaskTools xtaskTools;

	public void handlesmsWebSite(String source) throws IOException {

		logger.info("[蜀门/醉逍遥官网短信下发通道]请求资源：" + source);
		...
		try {
			...
			if (phones != null && phones.length > 1) {
				if (falg) {
					...
					// 短信群发
					String result = smsWebSiteTaskService.SMSGroupSending(group.toString(), sms.getMessage());
					if ("success".equalsIgnoreCase(result)) {
						...
						monitorlog.setLognote("群发短信失败! 服务器返回：" + result);
					}
				...
				if (falg) {
					...
					if (isPhone) {
						...
						String result = smsWebSiteTaskService.SMSSending(sms.getPhone(), sms.getMessage());
						if ("success".equalsIgnoreCase(result)) {
							logger.info("短信发送成功-> Phones: " + sms.getPhone());
							monitorlog.setLognote("发短信成功!");
							monitorlog.setLogtype(0);
						} else {
							logger.error("活动短信发送失败,忽略! phones: " + sms.getPhone() + "服务器返回：" + result);
							monitorlog.setLognote("发短信失败! 服务器返回：" + result);
						}
					} else {
			...
			logger.error("蜀门/醉逍遥官网短信下发通道_短信发送失败:其他异常：[" + sms.toString() + "]异常:", e);
			monitorlog.setLognote("[蜀门/醉逍遥官网短信下发通道]短信发送失败,忽略,其他异常:" + e.getMessage());
		} finally {
			try {
				xtaskTools.saveXtaskLogInfo(monitorlog);
			} catch (Exception ex) {
				logger.info("[蜀门/醉逍遥官网短信下发通道]日志保存失败!");
			}
		}
	}
