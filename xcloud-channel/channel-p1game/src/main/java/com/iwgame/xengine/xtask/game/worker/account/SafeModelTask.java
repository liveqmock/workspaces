 ...
 * @版本:   v1.0 
 */
public class SafeModelTask extends AbstractMQWorker {
	...
	public void handleMessage(String source){
		...
		logger.info("添加安全模式收到请求资源: " + source);
		...
		try {
			...
			if(Xvalidator.getInstance().validate(safeModelBean)){
				...
				monitorlog.setLognote("[添加安全模式]成功!");
			}else{
				monitorlog.setLognote("[添加安全模式]失败,请求的必要参数为空或者NULL,忽略此条消息...! 请求:"+source);
			}
		} catch (BadSqlGrammarException e) {
			monitorlog.setLognote("[添加安全模式]失败,发送到备份通道,连接异常"+e.getMessage());
			gameTaskServiceImpl.reSendMQBackupSafeModelActivate(source);
			...
			monitorlog.setLognote("[添加安全模式]失败,忽略,其他异常:"+e.getMessage());
			logger.error("其他异常信息,请求资源" + source, e);
		} finally{
			try {
				xtaskTools.saveXtaskLogInfo(monitorlog);
			} catch (Exception ex) {
				logger.error("[xtask-support][添加安全模式]存储日志异常！",ex);
			}
		}
	}
}
