...
(function($){
	...
	$.win = function(opt){		
		...
		win.append(head=$('<div class="win-head" />')
			.append(close=$('<div class="win-top-right"></div>'))
		...
		if (cfg.url!='') {
			if(!cfg.iframe){
				body.mload({url:cfg.url,maskCls:{background:'#FFF'},msg:cfg.msg,complete:function(){
					var contents=body.children().eq(0);
					contents.show();
				}});
			}else{
				var iframe_name = "xframe_"+(iframe_indexed ++);
			    var iframe = $("<iframe name="+iframe_name+" frameborder='0' style='width:100%;height:100%;position:absolute;z-index:100;overflow:auto' />");
				iframe.css("height",cfg.height - head.height()).css("width",cfg.width).appendTo(body);
			
				window.frames[iframe_name].document.write('<html><body leftmargin=0 topmargin=0 border=0 ><table border=0 width=100% height=100%><tr><td align=center valign=middle style="font-size:12px"><img src="img/load_page.gif" align="absmiddle"/>&nbsp;&nbsp;正在加载中,请稍候...</td></tr></table></body></html>');
				
				iframe.bind("load",function(){window.frames[iframe_name].window._selfiname=iframe_name;title.text($(window.frames[iframe_name].document).find("title").eq("0").html())});
	...
	$.closeWin = function (winname,callback){
	   var win_name = winname || window._selfiname;
