 ...
 * 
 */
public class OpenRaceTask extends AbstractMQWorker {

	private static final Logger logger = Logger.getLogger(OpenRaceTask.class);
	...
	public void handleMessage(final String text) {
		logger.info("######################    开启公会赛  开始  ######################");
		logger.info("从MQ服务器中获得开启公会赛消息");
		logger.info("消息为：" + text);
		try {
			contextService.loadCurrentRace();
			if (Context.get().hasOpenRace()) {
				contextService.initializeCache();
			}
		} catch (Exception e) {
			logger.error("开启公会赛异常\n" + e);
		} finally {
			logger.info("######################    开启公会赛  结束  ######################");
		}
	}
}
