 ...
 * 
 * @version 1.1.8, $Date: 2010-8-30$
 * 
 */
public abstract class ResHeaderUtil {
	
	private static final Logger logger = Logger.getLogger(ResHeaderUtil.class);
    ...
    public static ResHeader getResHeader(BindingProvider proxy) {
        ...
        if (headers == null || headers.size() == 0){
        	logger.error("The Response header does not exist!");
     ...
     *             If the Response header does not exist
     */
    public static ResHeader getResHeader(Object proxy) {
        return getResHeader((BindingProvider) proxy);
    }
    ...
    public static void printResHeader(ResHeader resH) {
    	...
    	logger.info("Status \t: " + resH.getStatus());
        logger.info("Desc \t: " + resH.getDesc());
        if (resH.getStatus() != 3) {
            logger.info("Oprs \t: " + resH.getOprs());
            logger.info("Oprtime\t: " + resH.getOprtime());
            logger.info("Quota \t: " + resH.getQuota());
            logger.info("Rquota \t: " + resH.getRquota());
        }
        List<Failure> failures = resH.getFailures();
        if (failures != null && failures.size() > 0) {
            for (int i = 0; i < failures.size(); i++) {
                logger.info("Failure [" + i + "] :");
                logger.info("\tcode \t\t: " + failures.get(i).getCode());
                logger.info("\tmessage \t: " + failures.get(i).getMessage());
                logger.info("\tposition \t: " + failures.get(i).getPosition());
                logger.info("\tcontent \t: " + failures.get(i).getContent());
            }
        }
    }
