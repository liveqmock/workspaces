 ...
 * @日期:	2012-10-29上午11:02:26
 * @版本:   v1.0
 */
public class BaiduAutoFetcherJob extends BaiduAuto{

	private final Logger logger = Logger.getLogger(BaiduAutoFetcherJob.class);
	...
	public void work() {
		...
		try {
			logger.info("[百度数据抓取任务] 开始。。。");
			logger.info("当前任务抓取时间范围：[ " + DateTimeUtil.formatDate3DateTimeStr(fetcherConfig.getFetcherBeginTime()) + " ]---[ "
			...
			if (null != adsconfigs && adsconfigs.size() > 0) {
				for (int i = 0; i < adsconfigs.size(); i++) {
					...
					try {
						...
						while (trycount-- > 0) {
							try {
								...
								logger.error("获取ReportId失败,webservice连接超时,我们将第 [" + trycount + "]次重试,请稍候...  \t 异常:", e);
								Thread.sleep(10000);
							}
						}
						...
						if (reprotIdResponse != null) {
							...
							while (trynum-- > 0) {
								try {
									...
									// 尝试一次停止30秒
									logger.error("获取fileUrlResponse失败,webservice连接超时,我们将第" + trynum + "次重试,请稍候...  \t 异常:", e);
								}
							}
							...
							if (null != fileUrlResponse) {
								...
								if (content != null && !"".equals(content)) {
									...
									for (int j = 0; j < list.size(); j++) {
										...
										try {
											...
											// 老孙
											reslut = adsService.sp_store_baidu_ads(map, adsConfig.getProductid());
								...
								status = -1;
								info += "获取广告失败,当前帐号: [ " + adsConfig.getUsername() + " ]!  百度帐号是否正确?,百度的webservice 是否连接超时?";
								logger.error(info);
							}
						} else {
							status = -1;
							info = " reprotIdResponse 对象为空，请检测百度帐号配置！";
							logger.error(info);
						}
					} catch (Exception e) {
						status = -1;
						info += e.getMessage();
						logger.error("Ignore Fetcher for Account : [ " + adsConfig.getUsername() + " ] !");
						logger.error(info, e);
					}
					...
					logger.info("-----------End fetcher [Account:" + adsconfigs.get(i).getUsername() + "]-------------\n\n\n");
				}
			} else {
		...
		for (int j = 0; j < adsHistoryBeans.size(); j++) {
			...
			logger.info("帐号: [" + bean.getUsername() + "]  抓取：[" + bean.getFetchecount() + "] 保存：[" + bean.getSavecount() + "] 是否成功?：[" + success + "]");
		}
		logger.info("--------------------------------------");
		logger.info("[百度数据抓取任务] 结束。。。\n\n\n\n\n");
	}
}
