/****************************************************************
 *  系统名称  ： ''
 *  文件名    ： FileUploadForSafeMode.java
 ...
 * @修改时间：2012-11-21 上午10:29:14
 */
public class FileUploadForSafeMode  implements FileProcessor,FileUploadMessages{
	private Logger logger4j = Logger.getLogger(FileUploadForSafeMode.class);
			...
			if(dbids.size()>globalResource.getFileMaxLine()){
				...
				for(String dbid:dbids){
					if(dbid.matches(RegexGwtHelper.STR_NUM_)){
						...
						if(docs!=null){
							Boolean isAdd = false;
							String username = solrHttpService.getAccountNameByAccountId(parameters.get("pid"),docs.getAccountid());
							if(StringUtils.isNotEmpty(username)){
								isAdd = true;
							}else{
								if("0".equals(parameters.get("flag"))){
									isAdd = true;
								}
							}
							if(isAdd){
								...
								bean.setDbid(dbid);
								bean.setFlag(Integer.parseInt(parameters.get("flag")));
								...
								try {
									...
									logger4j.error("批量添加安全模式,DBID:"+dbid+"错误提示:"+e);
									delcount++;
								}
							}else{
								logger4j.error("获取账户名称返回空,DBID:"+dbid);
								delcount++;
							}
						}else{
						...
						logger4j.error("dbid只能为-,数字,DBID:"+dbid);
						delcount++;
					}
				}
				if(count>0){
				...
				beans.clear();
				resultStr = "成功提交添加安全模式账号"+count+"条,忽略"+delcount+"条";
				if(count!=0){
