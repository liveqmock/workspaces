/****************************************************************
 *  系统名称  ： ''
 *  文件名    ： FileUploadForUnlockAccount.java
 ...
 * @修改时间：2012-11-20 下午12:24:13
 */
public class FileUploadForUnlockAccount implements FileProcessor,FileUploadMessages {
	private Logger logger4j = Logger.getLogger(FileUploadForUnlockAccount.class);
	...
	public void processUploadedFile(Map<String, String> parameters,
		...
		if(names!=null){
			if(names.size()>globalResource.getFileMaxLine()){
				...
				for(String name:names){
					if(name.matches(RegexGwtHelper.STR_ENG_NUM)){
						...
						try {
							...
							logger4j.error("批量解封,账号:"+name+"错误提示:"+e);
							delcount++;
						}
					}else{
				...
				beans.clear();
				resultStr = "成功提交解封账号"+count+"条,忽略"+delcount+"条";
				if(count!=0){
